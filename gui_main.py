...
class InvoiceSorterApp:
    def __init__(self, root):
        self.root = root
        self.root.title(f"{PROGRAM_NAME} {VERSION} - by {CREATOR}")
    ...
    def create_top_section(self):
        ...
        settings_frame = tk.LabelFrame(self.main_frame, text=" 창고 리스트 (마우스 휠 스크롤 가능) ", padx=10, pady=10)
        settings_frame.pack(fill=tk.X, pady=(0, 5))
        
        scroll_container = tk.Frame(settings_frame)
    ...
    def create_middle_section(self):
        ...
        tk.Button(middle_frame, text="📂 원본 엑셀 찾기", command=self.select_file).pack(anchor="w")
        self.file_path_label = tk.Label(middle_frame, text="선택된 파일 없음", fg="gray"); self.file_path_label.pack(anchor="w", padx=10)
        tk.Label(middle_frame, text="직접 붙여넣기:").pack(anchor="w", pady=(5, 2))
    ...
    def create_bottom_section(self):
        ...
        tk.Button(bottom_frame, text="🚀 분류 시작", bg="#4CAF50", fg="white", font=("맑은 고딕", 14, "bold"), height=2, command=self.start_sorting).pack(fill=tk.X, pady=(5, 10))
    ...
    def start_sorting(self):
        ...
        try:
            ...
            df = None
            if txt:
                try: df = pd.read_csv(io.StringIO(txt), sep='\t')
                except: df = pd.read_csv(io.StringIO(txt))
            elif self.selected_file_path: df = pd.read_excel(self.selected_file_path)
            ...
            if df is not None:
                ...
                path = os.path.join(os.path.expanduser("~"), "Desktop", "Taerang_sheets")
                if not os.path.exists(path): os.makedirs(path)
                fname = f"BR_통합결과_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                with pd.ExcelWriter(os.path.join(path, fname), engine='openpyxl') as writer:
                ...
                self.log(f" 💾 저장 완료: {fname}"); messagebox.showinfo("완료", "분류 완료 확인!"); os.startfile(path)
